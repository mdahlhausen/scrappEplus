module OsLib_HVAC
  ...
  def OsLib_HVAC.sortZones(model, runner, options = {})
    ...
    zones.each do |zone|
      ...
      #match to filter. Skip zone if name doesn't include zone_filter
      # Putting everything in Upper Case to make it case insensitive
      zone_filter = options["zoneFilter"]
      if !zone_filter.empty?
        if zone.name.to_s.upcase.include? zone_filter.to_s.upcase
          zonesFiltered << zone
        end
      else  
        zonesFiltered << zone
  ...
  def OsLib_HVAC.removeZoneEquipment(model, runner)
  ...
  def OsLib_HVAC.removeZoneFanExhaust(model, runner)
  ...
  def OsLib_HVAC.removeAirLoops(model, runner)
  ...
  def OsLib_HVAC.removePlantLoops(model, runner)
  ...
  def OsLib_HVAC.assignHVACSchedules(model, runner, options = {})
    ...
    if options["remake_schedules"] == true
    ...
    if options["primarySpaceType"] == "Classroom"
      ...
      if options["allHVAC"]["zone"] == "Radiant" or options["allHVAC"]["zone"] == "RadiantFloor" or options["allHVAC"]["zone"] == "RadiantCeiling"
    ...
    # low temp hot water
    if options["allHVAC"]["primary"]["heat"] == "LowTempHotWater" or options["allHVAC"]["secondary"]["heat"] == "LowTempHotWater"
      schedulesHVAC["hot_water"] = OsLib_Schedules.createComplexSchedule(model, {"name" => "LowTemp-HW-Loop-Temp-Schedule-110F",
                                                                                 "default_day" => ["All Days",[24,43.333]]})    
    end
    # chilled water
  ...
  def OsLib_HVAC.createPrimaryAirLoops(model, runner, options)
    ...
    if options["primaryHVAC"]["airLoopDivision"] == "Single"
      thermalZonesToAdd = []
      ...
      options["primaryHVAC"]["thermalZonesToAdd"] = thermalZonesToAdd
      options["primaryHVAC"]["loopKeyname"] = "Whole Bldg"      
      airloop_primary = OsLib_HVAC.createPrimaryAirLoop(model, runner, options)
      primary_airloops << airloop_primary
    else #make one for each story
      model.getBuildingStorys.sort.each do |building_story|
        ...
        options["primaryHVAC"]["thermalZonesToAdd"] = thermalZonesToAdd
        options["primaryHVAC"]["loopKeyname"] = building_story.name      
        airloop_primary = OsLib_HVAC.createPrimaryAirLoop(model, runner, options)
        primary_airloops << airloop_primary
      end
    end
  ...
  def OsLib_HVAC.createPrimaryAirLoop(model, runner, options)
    thermalZonesToAdd = options["primaryHVAC"]["thermalZonesToAdd"]
    loop_key_name = options["primaryHVAC"]["loopKeyname"]
    ...
    unless thermalZonesToAdd.empty?
      ...
      airloop_primary.setName("AEDG Air Loop HVAC #{loop_key_name}")
    ...
    model.getThermalZones.each do |zone|
      if options["zonesPrimary"].include? zone
        if options["zoneHVAC"] == "FanCoil"
          ...
          heating_coil = OpenStudio::Model::CoilHeatingLowTempRadiantVarFlow.new(model, options["radiant_heating_setpoint_schedule"])
          ...
          cooling_coil = OpenStudio::Model::CoilCoolingLowTempRadiantVarFlow.new(model, options["radiant_cooling_setpoint_schedule"])
          ...
          layers << concrete_3in = OpenStudio::Model::StandardOpaqueMaterial.new(model,"MediumRough",0.0762,2.31,2322,832)
          layers << concrete_1in = OpenStudio::Model::StandardOpaqueMaterial.new(model,"MediumRough",0.0254,2.31,2322,832)
        ...
        elsif options["zoneHVAC"] == "RadiantFloor"
          # create low temperature radiant object and add to thermal zone and radiant plant loops
          # create hot water coil and attach to radiant hot water loop
          #ADD OPTION TO USE ROOM THERMOSTAT
          heating_coil = OpenStudio::Model::CoilHeatingLowTempRadiantVarFlow.new(model, options["radiant_heating_setpoint_schedule"])
          heating_coil.setHeatingControlThrottlingRange(options["heating_control_throttling_range"])
          options["radiant_hot_water_plant"].addDemandBranchForComponent(heating_coil)
          # create chilled water coil and attach to radiant chilled water loop
          cooling_coil = OpenStudio::Model::CoilCoolingLowTempRadiantVarFlow.new(model, options["radiant_cooling_setpoint_schedule"])
          cooling_coil.setCoolingControlThrottlingRange(options["cooling_control_throttling_range"])
          options["radiant_chilled_water_plant"].addDemandBranchForComponent(cooling_coil)
          low_temp_radiant = OpenStudio::Model::ZoneHVACLowTempRadiantVarFlow.new(model, 
                                                                                  model.alwaysOnDiscreteSchedule(),
                                                                                  heating_coil,
                                                                                  cooling_coil)
          low_temp_radiant.setRadiantSurfaceType("Floors")
          low_temp_radiant.setHydronicTubingInsideDiameter(options["hydronic_tubing_inside_diameter"])
          low_temp_radiant.setTemperatureControlType(options["zoneHVACRadiantControl"])
          low_temp_radiant.addToThermalZone(zone)
          # create radiant floor construction and substitute for existing floor (interior or exterior) constructions
          # create materials for radiant floor construction
          layers = []
          slab_thickness = options["slabThickness"]
          if slab_thickness < 0.0508
            layer2_thickness = slab_thickness/2
            layer3_thickness = slab_thickness/2
          else
            layer3_thickness = 0.0127
            layer2_thickness = slab_thickness - layer3_thickness             
          end
          # ignore layer below insulation, which will depend on boundary condition
          layers << rigid_insulation_2in = OpenStudio::Model::StandardOpaqueMaterial.new(model,"Rough",0.0508,0.02,56.06,1210)
          layers << concrete_layer2 = OpenStudio::Model::StandardOpaqueMaterial.new(model,"MediumRough",layer2_thickness,2.31,2322,832)
          layers << concrete_layer3 = OpenStudio::Model::StandardOpaqueMaterial.new(model,"MediumRough",layer3_thickness,2.31,2322,832)
          # create radiant floor construction from materials
          radiant_floor = OpenStudio::Model::ConstructionWithInternalSource.new(layers)
          radiant_floor.setSourcePresentAfterLayerNumber(2)
          # assign radiant construction to zone floor
        ...
        elsif options["zoneHVAC"] == "RadiantCeiling"
          # create low temperature radiant object and add to thermal zone and radiant plant loops
          # create hot water coil and attach to radiant hot water loop
          heating_coil = OpenStudio::Model::CoilHeatingLowTempRadiantVarFlow.new(model, options["radiant_heating_setpoint_schedule"])
          options["radiant_hot_water_plant"].addDemandBranchForComponent(heating_coil)
          ...
          low_temp_radiant = OpenStudio::Model::ZoneHVACLowTempRadiantVarFlow.new(model, 
                                                                                  ...
                                                                                  heating_coil,
                                                                                  cooling_coil)
          low_temp_radiant.setRadiantSurfaceType("Ceilings")
          low_temp_radiant.setHydronicTubingInsideDiameter(0.012)
          low_temp_radiant.setTemperatureControlType(options["zoneHVACRadiantControl"])
          low_temp_radiant.addToThermalZone(zone)                
          # assign radiant construction to zone floor
          zone.spaces.each do |space|
            space.surfaces.each do |surface|
              if surface.surfaceType == "RoofCeiling"
                surface.setConstruction(options["zoneHVACRadiantConstruction"])
              end
            end
          end  
  ...
  def OsLib_HVAC.addZonesToAirLoop(model, runner, options)
    
    thermalZonesToAdd = options["thermalZonesToAdd"]
    airloop_name = options["airloop_name"]
    airloops = model.getAirLoopHVACs
    air_terminal_type = options["air_terminal_type"]
    ...
    thermalZonesToAdd.each do |zone|
      ...
      # remove from existing air loop, if applicable
      airloops.each do |a|
